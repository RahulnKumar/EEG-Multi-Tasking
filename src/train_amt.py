...
def test_accuracy_multitask_net():
    ...
    try:
        checkpoint = torch.load('models/adversarial_multi_task_train_batch/convnet.pth', map_location=torch.device(gpu if torch.cuda.is_available() else 'cpu'))
...
def adversarial_multi_task_train():
    ...
    #         traceback.print_exc()
        ...
        for epoch in range(num_epochs):
            ...
            for i, (images, labels, subjects) in enumerate(train_loader):
                ...
                loss1 = criterion1(outputs1[subjects[0].item()-1], labels)
    ...
    #             model1.to(device)
                ...
                torch.save({'weights': model2.state_dict()},'models/adversarial_multi_task_train_batch/tasknet.pth')
                torch.save({'weights': model1.state_dict()},'models/adversarial_multi_task_train_batch/convnet.pth')
...
def store_results(accuracy):
    try:
        with open('results/amt.json') as file:
            accuracy_list = json.load(file)
    #         print(accuracy_list)
        with open('results/amt.json', 'w') as file:
